...
2. When one of these governs a subexpression, such as a?, (expr )+, [0-9]+, there is a minimum number of matches that 
   are required before it can be consider ed successful and maximum attempt it will ever attempt to match, 
3. They always attempt to match as many times as they can, up to that maximum allowed.
4. The only time they settle for anything less than their maximum allowed is when matching too much ends up causing 
   some later part of the regex to fail.
"""
...
# Word Boundary (\b) [a-zA-Z0-9]
# "start of word" boudry is simply the position where a sequence of alphanumeric characters begins;
...
match = re.findall(r"[A-Z]", 'Cascading Style Sheets')
# --------------------------------------------------------------------------
...
re.findall(r"Inbox(?:\(\d+\))", "Inobx(10)")
re.findall(r"Inbox(?:\(\d+\))", "Inobx")
# ------------------------------------------------------------------------------------------------------------
...
# Matches the string that ends with 2 digits
re.findall(".*[0-9][0-9]$", "this is a ending with 26")
re.findall(".*[0-9][0-9]$", "26")
